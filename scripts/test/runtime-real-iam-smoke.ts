...
async function seedSystem(databaseUrl: string): Promise<void> {
  ...
  try {
    ...
    await execute(connection, "INSERT INTO system_config_release (id, release_key, status, digest, published_at, created_at, updated_at) VALUES (?, 'iam-smoke', 'active', ?, ?, ?, ?)", [releaseId, "1".repeat(64), now, now, now]);
...
async function main(): Promise<void> {
  ...
  if (!baseDatabaseUrl || !redisUrl || !iamBaseUrl || !workloadToken) {
    throw new Error("TEST_DATABASE_URL, TEST_REDIS_URL, IAM_BASE_URL and IAM_WORKLOAD_TOKEN are required");
  }

  const systemSchema = await readFile(new URL("../../database/migrations/001_system.sql", import.meta.url), "utf8");
  const systemDatabase = await createDatabase(baseDatabaseUrl, "kokoro_system_iam", systemSchema);
  ...
  try {
    ...
    system = await createSystemRuntime({
      ...
      redisUrl,
      redisNamespace,
      binding: new IamTenantBindingClient(iamBaseUrl, workloadToken),
    });
