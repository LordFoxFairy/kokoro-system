...
function connectionOptions(value: string, database: string): ConnectionOptions {
  ...
  return { connectionString: url.toString(), options: "-c search_path=public" };
}
...
function themeSource(value: unknown): string | null {
...
async function seed(databaseUrl: string): Promise<void> {
  ...
  const connection = new Client(
    connectionOptions(databaseUrl, decodeURIComponent(dbUrl.pathname.slice(1))),
  );
  try {
    ...
    const insertRecord = async (
      ...
      await execute(
        connection,
        `INSERT INTO system_config_record (id, tenant_id, module_key, scope_type, scope_id, product_id, locale, config_key, schema_version, value_json, status, config_version, release_id, digest, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 1, $9, 'active', $10, $11, $12, $13, $14)`,
        [
          input.id,
          input.tenantId,
          input.moduleKey,
    ...
    await execute(
      connection,
      `INSERT INTO system_product (id, product_key, name, status, created_at, updated_at) VALUES ($1, $2, $3, 'active', $4, $5)`,
      [productId, productKey, "Kokoro", now, now],
    );
    ...
    await execute(
      ...
      [siteA, tenantA, "main-a", "Tenant A", now, now],
    );
    await execute(
      connection,
      `INSERT INTO system_site (id, tenant_id, site_key, display_name, status, version, created_at, updated_at) VALUES ($1, $2, $3, $4, 'active', 1, $5, $6)`,
      [siteB, tenantB, "main-b", "Tenant B", now, now],
    );
    await execute(
      connection,
      `INSERT INTO system_site_host (id, tenant_id, site_id, hostname, status, created_at, updated_at) VALUES ($1, $2, $3, $4, 'active', $5, $6)`,
      [randomUUID(), tenantA, siteA, "tenant-a.example.test", now, now],
    );
    await execute(
      connection,
      `INSERT INTO system_site_host (id, tenant_id, site_id, hostname, status, created_at, updated_at) VALUES ($1, $2, $3, $4, 'active', $5, $6)`,
      [randomUUID(), tenantB, siteB, "tenant-b.example.test", now, now],
    );
    await execute(
      connection,
    ...
    await execute(
      ...
      [randomUUID(), tenantA, productId, releaseId, now, now],
    );
    await execute(
      connection,
      `INSERT INTO system_release_binding (id, scope_type, scope_id, product_id, release_id, status, created_at, updated_at) VALUES ($1, 'tenant', $2, $3, $4, 'active', $5, $6)`,
      [randomUUID(), tenantB, productId, releaseId, now, now],
    );
    await insertRecord({
      id: randomUUID(),
      tenantId: null,
...
async function main(): Promise<void> {
  ...
  if (!baseDatabaseUrl || !testRedisUrl)
  ...
  const systemServer = createHttpServer(
    runtime.service,
    async () => {
  ...
  try {
    ...
    const cached = await a.getRuntimeManifest({
      productId: productKey,
      locale: "en-US",
      surfaceId: surfaceA,
    });
    const tenantOnly = await a.getRuntimeManifest({
      productId: productKey,
      locale: "en-US",
    });
    const otherTenant = await b.getRuntimeManifest({
      productId: productKey,
      locale: "en-US",
    });
    ...
    const missingServiceAuth = await requestStatus(
      `${system.url}/v1/system/runtime-manifest?product_id=${productKey}`,
      { headers: { "x-kokoro-tenant-id": tenantA } },
    );
    assert(
      missingServiceAuth.status === 403,
      "missing service auth did not return 403",
    );
    const missingTenant = await requestStatus(
      `${system.url}/v1/system/runtime-manifest?product_id=${productKey}`,
      {
        headers: {
    ...
    assert(
      missingTenant.status === 400,
      "missing tenant header did not return 400",
    );
    const missingProduct = await requestStatus(
      `${system.url}/v1/system/runtime-manifest`,
      {
        headers: {
          "x-kokoro-service": "web-bff",
          "x-kokoro-internal-secret": bffServiceToken,
    ...
    await expectSdkStatus(
      () =>
        mismatch.getRuntimeManifest({ productId: productKey, locale: "en-US" }),
      404,
    ...
    await expectSdkStatus(
      () => a.getRuntimeManifest({ productId: productKey, locale: "en-US" }),
      503,
...
async function expectSdkStatus(
  operation: () => Promise<RuntimeManifest>,
  status: number,
  label: string,
): Promise<void> {
  try {
    ...
    if (error instanceof SystemSdkError && error.status === status) return;
    if (error instanceof SystemSdkError)
      throw new Error(
        `${label} returned HTTP ${error.status} (${error.code}), expected ${status}`,
        { cause: error },
      );
    throw new Error(`${label} did not return HTTP ${status}`, { cause: error });
  }
