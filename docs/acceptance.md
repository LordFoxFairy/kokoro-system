...
| Service auth | `manifest`、RPC manifest 和 `/system/*` 始终要求 `x-kokoro-service: web-bff` 与匹配的 internal secret 或 Bearer；缺 token 返回 `503 service_auth_not_configured`，错误认证返回 `403 service_auth_failed`；healthz/readyz 仍公开 |
| Tenant auth | 配置 service auth 的控制面请求仍要求 IAM tenant binding；缺 tenant 不从 query/body 回退 |
