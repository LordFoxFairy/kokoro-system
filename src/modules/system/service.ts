...
function requireKey(key: string): string { if (!key.trim() || key.length > 128) throw new SystemDomainError("INVALID_ARGUMENT", "idempotency key is invalid"); return key; }
...
export class SystemControlService {
  ...
  public async createWorkspace(context: TenantRequestContext, input: WorkspaceInput, key: string): Promise<Workspace> { requirePermission(context, "system:write"); validateText("workspace_key", input.workspaceKey); validateText("name", input.name); validateText("site_id", input.siteId); return this.mutate(context, key, input, () => this.repository.createWorkspace(context, input)); }
  public async getPolicy(context: TenantRequestContext, siteId: string): Promise<SitePolicy> { requirePermission(context, "system:read"); const value = await this.repository.getPolicy(context, siteId); if (!value) throw new SystemDomainError("NOT_FOUND", "site policy not found", 404); return value; }
  public async putPolicy(context: TenantRequestContext, siteId: string, input: Omit<SitePolicy, "id" | "tenantId" | "siteId" | "updatedAt">, key: string): Promise<SitePolicy> { requirePermission(context, "system:write"); return this.mutate(context, key, { siteId, ...input }, () => this.repository.putPolicy(context, siteId, input)); }
  public async createRelease(context: TenantRequestContext, input: ReleaseInput, key: string): Promise<ConfigRelease> { requirePermission(context, "system:write"); return this.mutate(context, key, input, () => this.repository.createRelease(context, input)); }
  public async listConfigs(context: TenantRequestContext, page: PageRequest): Promise<Page<SystemConfig>> { requirePermission(context, "system:read"); validPage(page); return this.repository.listConfigs(context, page); }
  public async upsertConfig(context: TenantRequestContext, input: ConfigInput, key: string): Promise<SystemConfig> { requirePermission(context, "system:write"); validateText("module_key", input.moduleKey); validateText("config_key", input.configKey); if (input.scopeType === "tenant" && input.scopeId !== context.tenantId) throw new SystemDomainError("FORBIDDEN", "tenant scope does not match request context", 403); if (input.scopeType === "global") requirePermission(context, "system:publish"); return this.mutate(context, key, input, () => this.repository.upsertConfig(context, input)); }
  public async validateRelease(context: TenantRequestContext, id: string, key: string): Promise<ConfigRelease> { requirePermission(context, "system:publish"); return this.transition(context, id, "validated", key); }
  public async publishRelease(context: TenantRequestContext, id: string, key: string): Promise<ConfigRelease> { requirePermission(context, "system:publish"); return this.transition(context, id, "published", key); }
  public async retireRelease(context: TenantRequestContext, id: string, key: string): Promise<ConfigRelease> { requirePermission(context, "system:publish"); return this.transition(context, id, "retired", key); }

  private async transition(context: TenantRequestContext, id: string, next: ConfigRelease["status"], key: string): Promise<ConfigRelease> {
