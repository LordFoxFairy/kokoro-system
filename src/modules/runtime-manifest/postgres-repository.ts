...
async function resolveProductId(client: { query<Row>(sql: string, values?: readonly unknown[]): Promise<{ rows: readonly Row[]; affectedRows: number }> }, productId: string): Promise<string | null> {
  const result = uuidPattern.test(productId)
    ? await client.query<Row>(`SELECT id FROM system_product WHERE id = ? AND status = 'active' AND deleted_at IS NULL LIMIT 1`, [productId])
    : await client.query<Row>(`SELECT id FROM system_product WHERE product_key = ? AND status = 'active' AND deleted_at IS NULL ORDER BY updated_at DESC, id DESC LIMIT 1`, [productId]);
  const value = result.rows[0]?.id;
  return value === undefined || value === null ? null : String(value);
}

function emptyManifest(input: Readonly<{ context: TenantRequestContext; productId: string; locale: string }>): RuntimeManifest {
  const result = { tenantId: input.context.tenantId, productId: input.productId, locale: input.locale, navigation: [], localeNamespaces: [], theme: {}, featureFlags: [], references: [], configVersion: "0", releaseId: null, digest: "" };
...
export class PostgresSystemRepository implements SystemRepository {
  ...
  public async getManifest(input: Readonly<{ context: TenantRequestContext; productId: string; locale: string }>): Promise<RuntimeManifest> {
    ...
    try {
      ...
      const bindings = await client.query<Row>(`SELECT release_id FROM system_release_binding WHERE scope_type = 'tenant' AND scope_id = ? AND product_id = ? AND status = 'active' ORDER BY updated_at DESC, id DESC LIMIT 1`, [input.context.tenantId, productUuid]);
      const binding = bindings.rows[0];
      const releaseId = binding?.release_id === undefined ? null : String(binding.release_id);
      const records = await client.query<Row>(`SELECT id, module_key, scope_type, scope_id, product_id, locale, config_key, value_json, config_version, release_id, digest FROM system_config_record WHERE status = 'active' AND deleted_at IS NULL AND (release_id = ? OR release_id IS NULL) AND (tenant_id = ? OR tenant_id IS NULL) AND (locale = ? OR locale IS NULL) AND (product_id = ? OR product_id IS NULL) AND ((scope_type = 'surface' AND scope_id = ?) OR (scope_type = 'tenant' AND scope_id = ?) OR (scope_type = 'product' AND scope_id = ?) OR scope_type = 'global') ORDER BY module_key, config_key, id`, [releaseId, input.context.tenantId, input.locale, productUuid, input.context.surfaceId, input.context.tenantId, productUuid]);
