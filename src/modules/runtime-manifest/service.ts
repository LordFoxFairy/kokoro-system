...
import type { ManifestCache, SiteHostResolver, SystemRepository } from "./ports.js";
...
export class RuntimeManifestService {
  public constructor(private readonly repository: SystemRepository, private readonly cache: ManifestCache, private readonly hostResolver: SiteHostResolver) {}
  public async get(input: Readonly<{ context: TenantRequestContext; productId: string; locale: string; host: string }>): Promise<RuntimeManifest> {
    await this.hostResolver.verify({ context: input.context, host: input.host });
