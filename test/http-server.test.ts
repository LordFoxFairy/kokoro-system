    ...
    return await new Promise((resolve, reject) => {
      const request = httpRequest(
        ...
        (response) => {
          ...
          response.on("end", () => {
            const responseBody: unknown = JSON.parse(body);
            resolve({
...
describe("System HTTP contract", () => {
  it("returns 400 when the tenant context header is missing", async () => {
    ...
    await expect(
      getJson(server, "/v1/system/runtime-manifest?product_id=product-a", {
      ...
      body: {
        error: {
          ...
          message: "x-kokoro-tenant-id is required",
        },
  ...
  it("returns 503 when readiness dependencies fail and preserves request id", async () => {
    const server = createHttpServer(
      serviceStub(),
      async () => {
        throw new Error("dependency fixture down");
      },
      { bffServiceToken: "service-token" },
    );
    await expect(getJson(server, "/readyz")).resolves.toMatchObject({
      status: 503,
      body: {
        error: { code: "SYSTEM_UNAVAILABLE", message: "system unavailable" },
        meta: { request_id: expect.any(String) },
      },
    });
  });
  ...
  it("keeps health and readiness public when BFF service auth is enabled", async () => {
    ...
    await expect(getJson(server, "/healthz")).resolves.toMatchObject({
      status: 200,
      body: {
        data: { status: "ok", service: "kokoro-system" },
        meta: { request_id: expect.any(String) },
      },
    });
    await expect(getJson(server, "/readyz")).resolves.toMatchObject({
      status: 200,
      body: {
        data: { status: "ready", service: "kokoro-system" },
        meta: { request_id: expect.any(String) },
      },
    });
  });
  ...
  it("requires web-bff service auth before runtime manifest access", async () => {
    ...
    await expect(
      getJson(
        server,
        "/v1/system/runtime-manifest?product_id=product-a",
        tenant,
      ),
    ).resolves.toEqual({ status: 403, body: expected });
    await expect(
      getJson(server, "/v1/system/runtime-manifest?product_id=product-a", {
        ...tenant,
        "x-kokoro-service": "not-web-bff",
        "x-kokoro-internal-secret": "service-token",
      }),
    ).resolves.toEqual({ status: 403, body: expected });
    await expect(
    ...
    await expect(
      getJson(server, "/v1/system/runtime-manifest?product_id=product-a", {
        ...tenant,
        "x-kokoro-service": "web-bff",
        "x-kokoro-internal-secret": "wrong-token",
      }),
    ).resolves.toEqual({ status: 403, body: expected });
    await expect(
      getJson(server, "/v1/system/runtime-manifest?product_id=product-a", {
        ...tenant,
        "x-kokoro-service": "web-bff",
        "x-kokoro-service-token": "service-token",
      }),
    ).resolves.toEqual({ status: 403, body: expected });
  });
  ...
  it("does not expose an unauthenticated System path when service auth is enabled", async () => {
    ...
    await expect(getJson(server, "/v1/system/not-a-route")).resolves.toEqual({
      status: 403,
      body: {
        error: {
          code: "service_auth_failed",
