...
-- Tenant IDs are opaque cross-repository context values. Site and host ownership remain entirely inside System; no cross-service relational constraints are used.
...
CREATE TABLE IF NOT EXISTS system_config_release (
  id UUID PRIMARY KEY, tenant_id TEXT, release_key VARCHAR(128) NOT NULL, status VARCHAR(32) NOT NULL,
...
CREATE TABLE IF NOT EXISTS system_release_binding (
  id UUID PRIMARY KEY, scope_type VARCHAR(32) NOT NULL, scope_id TEXT, product_id UUID, release_id UUID NOT NULL,
...
CREATE TABLE IF NOT EXISTS system_config_record (
  id UUID PRIMARY KEY, tenant_id TEXT, module_key VARCHAR(64) NOT NULL, scope_type VARCHAR(32) NOT NULL,
  scope_id TEXT, product_id UUID, locale VARCHAR(32), config_key VARCHAR(160) NOT NULL, schema_version INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS system_audit_event (
  id UUID PRIMARY KEY, tenant_id TEXT, actor_id UUID, command_id UUID, kind VARCHAR(128) NOT NULL,
...
CREATE TABLE IF NOT EXISTS system_site (
  id UUID PRIMARY KEY, tenant_id TEXT NOT NULL, site_key VARCHAR(128) NOT NULL,
  display_name VARCHAR(160) NOT NULL, status VARCHAR(32) NOT NULL,
...
CREATE UNIQUE INDEX IF NOT EXISTS system_site_host_site_hostname_uidx
  ON system_site_host (tenant_id, site_id, hostname);
CREATE INDEX IF NOT EXISTS system_site_host_tenant_idx ON system_site_host (tenant_id, status, hostname, site_id);
...
CREATE TABLE IF NOT EXISTS system_command_receipt (
  id UUID PRIMARY KEY, tenant_id TEXT NOT NULL, idempotency_key VARCHAR(128) NOT NULL,
